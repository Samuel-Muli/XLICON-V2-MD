...
const handler = async (m, {conn, text, args, usedPrefix, command}) => {
  if (!text) throw `_*Shotgun_Suppressor TIKTOK DL*_\n\n*_Paste a tiktok link._*\n\n*_Example:_* _${usedPrefix + command} Url here_`;
  if (!/(?:https:?\/{2})?(?:w{3}|vm|vt|t)?\.?tiktok.com\/([^\s&]+)/gi.test(text)) throw `_*Shotgun_Suppressor TIKTOK DL*_\n\n*_Enter a TikTok link._*\n\n*_Example:_* _${usedPrefix + command} Url here_`;
...
async function tiktokdlF(url) {
  if (!/tiktok/.test(url)) return `_*Shotgun_Suppressor TIKTOK DL*_\n\n*_Paste a tiktok link._*\n\n*_Example:_* _${usedPrefix + command} Url here_`;
